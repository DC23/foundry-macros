 ...
 * Can be used to edit any properties. 
 * I needed to change animation intensity, and the min/max darkness settings 
 * on a swamp map, so that's what I've saved here :)
 *
...
const updates = lights.map(l => ({
    ...
    'config.darkness.max': 0.7,
}))
if (updates) {
    await AmbientLightDocument.updateDocuments(updates, {
        parent: canvas.scene,
    })
}
